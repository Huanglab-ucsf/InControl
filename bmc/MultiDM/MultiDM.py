...
class Control(inLib.Device):
    ...
    def addOther(self, data, on_geometry_changed=None, args=()):
        ...
        data_new = np.fliplr(data_new) # added by Dan on 0628
...
class Mirror():
    ...
    def whereSegment(self, seg):
        ...
        xStart = int(self.borders[unraveled[0]])
        xStop = int(self.borders[unraveled[0]+1])
        yStart = int(self.borders[unraveled[1]])
        yStop = int(self.borders[unraveled[1]+1])
